angular.module('starter.controllers')
  .controller('AnnouncementListController', ['$scope', '$rootScope', '$state', '$ionicPopup', '$ionicActionSheet', function ($scope, $rootScope, $state, $ionicPopup, $ionicActionSheet) {
      ...
      popup.then(function (resolution) {
        if (resolution && resolution.length > 0) {
          for (var i = 0; i < $scope.announcements.length; i++) {
            if ($scope.announcements[i].announcementId == commentTarget) {
              $scope.announcements[i].comments.push({
                commentId: 12,
                content: resolution,
                user: {
                  displayName: 'test user 3',
    ...
    $scope.like = function (likeTarget) {
      for (var i = 0; i < $scope.announcements.length; i++) {
        var target = $scope.announcements[i];
        if (target.announcementId == likeTarget) {
          if (target.liked) {
            target.likeCount--;
            target.liked = false;
          } else {
  ...
  .controller('AnnouncementDetailController', ['$scope', '$rootScope', '$state', '$stateParams', function ($scope, $rootScope, $state, $stateParams) {
    $scope.announcements = [{
      announcementId: 1,
      title: 'quest title 1',
    ...
    var targetAnnouncement = parseInt($stateParams.announcementId);
    for(var i = 0; i < $scope.announcements.length; i++) {
      if($scope.announcements[i].announcementId == targetAnnouncement) {
        $scope.announcement = $scope.announcements[i];
        break;
      }
    }
  }])
